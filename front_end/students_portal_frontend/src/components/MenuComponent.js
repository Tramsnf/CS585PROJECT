...
const MenuComponent = (props) => {
  ...
  const [getProfiles, searchProfiles] = useLazyQuery(PROFILE_HIGHLIGHT,{
    ...
    variables: {profile : user.type },
  });

  useEffect(() => {
    console.log("refetch highligts")
            ...
            <>  
              <div className='p-3 pb-0'>
                ...
                <div className='bg-white border border-gray border-opacity-20 rounded-lg'>
                    {!!searchProfiles.data.profileHighlights.recentlyContacted.length && searchProfiles.data.profileHighlights.recentlyContacted.map((value, index)=>
                          user.type==="student"?
              ...
              <div className='p-3'>
                ...
                <div className='bg-white border border-gray border-opacity-20 rounded-lg'>
                    {!!searchProfiles.data.profileHighlights.frequentlyContacted.length &&  searchProfiles.data.profileHighlights.frequentlyContacted.map((value, index)=>
                              user.type==="student"?
                              <ContactMenuComponent
                                key={index}
                                name={value.lecturer.profile.userName}
