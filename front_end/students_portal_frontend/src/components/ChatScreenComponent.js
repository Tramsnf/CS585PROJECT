...
const ChatScreenComponent = (props) => {
  ...
  const getUserSpaceDto = useQuery(GET_USER_CHAT_SPACE,{
    fetchPolicy: 'network-only', // network request no cache
    variables: {profile : profile, refId : spaceRefId},
  });

  useEffect(() => {
    if(getUserSpaceDto.data && !getUserSpaceDto.error && !getUserSpaceDto.loading){
      console.log(getUserSpaceDto.data)
  ...
  useEffect(()=>{
    ...
    if(spaceDto && spaceDto.getDetailedChatSpace){
      getUserSpaceDto.refetch()
    }
  },[urlParams.profile, activeProfile.data,])
